...
function extractCommonScripts(log, options, route, bundlePath, placement, downloads) {
    ...
    if (options.strategy === 'manual' && options.files && options.files[placement]) {
        const manualFiles = options.files[placement];
        ...
        for (const path of htmlPaths) {
            ...
            for (let i = srcs.length - 1; i >= 0; --i) {
                const src = srcs[i];
                const formattedSrc = route.format(src);
                
                if (manualFiles.includes(src) || manualFiles.includes(formattedSrc)) {
